...
class App {
  ...
  constructor(routes: any, repository: any, variables: any, service: any) {
    this.app = express();
  ...
  async start() {
    ...
    this.app.use(
      "/api/v1/docs",
      swaggerUi.serve,
      swaggerUi.setup(swaggerDoc.getDoc())
    );
    ...
    if (!Number(process.env.IS_HTTPS)) {
      ...
      listener = httpsServer.listen(this.port, () =>
        this.service.logger.info(`Https app listening on port: ${this.port}`)
      );
    }
