...
export class SendPartialSocket {
  ...
  async sendPartialSocket({ key, uid, user_id, cb, totalData }: any) {
  ...
  async _doRecursive({
    ...
    if (uid !== this.cacheData.getCacheV2(keyUid)) {
      ...
      this.io.emit(keyUid.split("/")[0], {
        ...
        user: keyUid.split("/")[1],
        uid: uid,
        message: "Kesalahan Session pada user " + keyUid.split("/")[1],
      });
    ...
    this.io.emit(keyMsg, {
      status: "PROGRESS",
      data: result,
      progress: skip + result.length,
      total: totalData,
      user: user_id,
      uid: uid,
    });

    return await this._doRecursive({
      cb,
      totalData,
      keyUid,
      uid,
  ...
  async sendPartialSocketGroupRecursive({ key, uid, user_id, cb }: any) {
    const keyUid = `${key}/${user_id}`;
    this.cacheData.storeCacheV2(keyUid, uid);
  ...
  async _doRecursiveGroup({
    ...
    if (!result.groupLength) {
      throw new Error(
        "please provide groupLength at return value of partial socket callback!"
      );
    }
  ...
  async sendPartialSocketRecursion({ key, uid, user_id, cb }: any) {
    const keyUid = `${key}/${user_id}`;
    this.cacheData.storeCacheV2(keyUid, uid);
  ...
  async _doRecursiveTillEmpty({
    ...
    this.io.emit(keyMsg, {
      ...
      data: result,
      progress: skip + result.length,
      total: skip + result.length,
      user: user_id,
      uid: uid,
    });

    return await this._doRecursiveTillEmpty({
      cb,
      keyUid,
      uid,
      skip: progress,
